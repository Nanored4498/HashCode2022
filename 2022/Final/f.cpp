...
int main() {
	...
	double w_coeff = 2.*sts[0].getD(DD/4)/double(l[P0]);
	...
	while(true) {
		...
		for(int i = 0; i < DD; ++i) {
			for(int j = 0; j < DD; ++j) {
				...
				int w = l[P0]-ww-sts[P].getC(x2-x, 0)-sts[P].getC(y2-y, 0)-sts[P].getC(x2, 0)-sts[P].getC(y2, 0);
				if(w <= 0) continue;
				for(bool mm : {false, true}) {
					vector<string> pth0;
					if(mm) sts[P].move2(x2-x, y2-y, pth0);
					else sts[P].move1(x2-x, y2-y, pth0);
					vi ind0; for(auto [u, v] : path(x, y, pth0)) ind0.insert(ind0.end(), gi[u-mic][v-mic].begin(), gi[u-mic][v-mic].end());
		...
		if(best_score < 0 || (ww+600>l[P0] && T*best_score < 1.6e6)) {
			if(!ww) break;
			int t = 1e9;
