		...
		for(int t : ts) {
			...
			for(const Point &pt : P[t]) {
				...
				while(!Q.empty() && !found) {
					...
					for(char c : vs) {
						...
						if(j != i && until[p.x][p.y] > l) {
							...
							if(distance(p, arms[i].cur[0]) > 2*distance(p, arms[j].cur[0])) continue;
							if(distance(P[t][0], arms[i].cur.back()) + arms[j].path.size() > distance(P[t][0], arms[j].cur.back()) + arms[j].path.size()) continue;
							l2 = until[p.x][p.y];
						}
						++ l2;
				...
				for(char c : add) {
					...
					a.path += string(dist[p.x][p.y]-dist[a.cur.back().x][a.cur.back().y]-1, 'W')+c;
					a.cur.push_back(p);
				}
				if(a.path.size() > L) {
					bad = true;
